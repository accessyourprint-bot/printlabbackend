...
print("Step 1: Cleaning and Populating Database...")
...
INSERT INTO orders (id, order_number, user_id, shop_id, status, delivery_type, grand_total, printing_cost, created_at) 
VALUES 
('11111111-1111-1111-1111-111111111111', 'ORD-1-0001', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'shop-001', 'pending', 'self_pickup', 690, 690, NOW()),
('22222222-2222-2222-2222-222222222222', 'ORD-H1', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'shop-001', 'delivered', 'self_pickup', 4330, 4330, NOW() - INTERVAL '1 day'),
('33333333-3333-3333-3333-333333333333', 'ORD-H2', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'shop-001', 'delivered', 'self_pickup', 3390, 3390, NOW() - INTERVAL '2 days');

INSERT INTO order_files (id, order_id, user_id, storage_key, nonce, original_filename, content_type, file_size_bytes, page_count, file_type, print_color, copies, expires_at) 
VALUES 
(gen_random_uuid(), '11111111-1111-1111-1111-111111111111', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'k1', 'n1', 'Assignment_Final.pdf', 'application/pdf', 1024, 5, 'document', 'black_white', 3, NOW() + INTERVAL '7 days'),
(gen_random_uuid(), '22222222-2222-2222-2222-222222222222', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'k2', 'n2', 'Project_Report.pdf', 'application/pdf', 2048, 10, 'document', 'color', 1, NOW() + INTERVAL '7 days'),
(gen_random_uuid(), '33333333-3333-3333-3333-333333333333', (SELECT id FROM users WHERE email='admin@altprint.in' LIMIT 1), 'k3', 'n3', 'Photos.zip', 'application/zip', 5000, 1, 'image', 'color', 1, NOW() + INTERVAL '7 days');

INSERT INTO delivery_persons (id, name, phone, vehicle_number, current_status, shop_id) 
VALUES 
...
UPDATE feature_flags SET enabled = true WHERE feature_name = 'login_register';
"""

with open("temp_data.sql", "w") as f:
    f.write(sql_commands)

subprocess.run(["docker", "exec", "-i", "altprint_postgres", "psql", "-U", "altprint", "-d", "altprint_db", "-f", "/workdir/temp_data.sql"], shell=True)
os.remove("temp_data.sql")

# --- PART 2: RESTORE THE PREMIUM UI ---
print("Step 2: Restoring Premium UI...")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .lb_card h2 { font-size: 1.6rem; font-weight: 900; margin-bottom: 5px; }
        ...
        .si_box { width: 44px; height: 44px; background: #fde8e0; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; cursor: pointer; position: relative; }
        .sn_text { font-size: 1.1rem; font-weight: 800; }
        ...
        .jp_card { background: #fff; border-radius: 20px; border: 1px solid #eee; overflow: hidden; }
...
<body>
    <div id="loginView"><div class="lw_bg"><div class="lb_card"><h2>PrintLab Outlet</h2><p style="color:#94a3b8; font-size:.9rem; margin-bottom:30px;">Sign in to manage your shop</p><input class="fi" id="em" value="admin@altprint.in"><input class="fi" id="pw" type="password" value="AltPrint2024!"><button class="lpb" onclick="handleLogin()">Sign In</button><div id="lerr" style="color:#e84c1e; text-align:center; font-size:.85rem; margin-top:15px; font-weight:800;"></div></div></div></div>
    <div id="dashView" class="hidden">
        <div class="tb">
            <div class="tbl"><div class="si_box" onclick="toggleMenu('sdrop')">🏠<div id="sdrop" class="drop_menu s_left hidden"><div class="m_item" onclick="openAgents()">🚚 Logistics</div><div class="m_item" onclick="openMo('setMo')">⚙️ Settings</div><div class="m_item" onclick="handleLogout()" style="color:#e84c1e;">➔ Logout</div></div></div><div><div class="sn_text">Print Lab Koramangala</div><div style="font-size:.75rem; color:#94a3b8; font-weight:700;">Dashboard</div></div></div>
            <div class="tbr"><button class="btn_top" onclick="openAgents()">👥 Delivery Team</button><button class="btn_top" onclick="openPerf()">📊 Performance Status</button><button class="btn_top tbtn" onclick="toggleMenu('tdrop')">🎫 Ticket Raise ▼<div id="tdrop" class="drop_menu t_right hidden"><div class="m_item" onclick="openTicketForm('Customer')">👤 Customer Ticket</div><div class="m_item" onclick="openTicketForm('Stock')">📦 Stock Ticket</div><div class="m_item" onclick="openTicketForm('Technical')">🛠️ Technical Issue</div></div></button></div>
        </div>
    ...
    <div id="perfModal" class="mo hidden"><div class="mb"><div class="mh"><h3>Performance Status</h3><button onclick="closeMo('perfModal')" style="cursor:pointer; border:none; background:none; font-size:1.5rem; color:#cbd5e1;">✕</button></div><div class="mbody"><div id="pStats" class="stats_grid"></div><h4 style="margin-bottom:20px; font-weight:900;">Earnings by Print Type (Today)</h4><div id="catBars"></div><hr style="border:none; border-top:1.5px solid #f1f5f9; margin-bottom:35px;"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;"><h3 style="font-size:1.2rem; font-weight:900;">Sales History</h3><div class="tt_box"><button class="tab_b on" id="shAll" onclick="updateHistory('all', this)">All Types</button><button class="tab_b" onclick="updateHistory('Bw', this)">B/W</button></div></div><canvas id="pChart" height="150"></canvas><h4 style="margin:35px 0 20px; font-weight:900; border-bottom:2px solid #f1f5f9; padding-bottom:12px;">Daily History</h4><div id="hList"></div></div></div></div>
    ...
    <div id="setMo" class="mo hidden"><div class="mb" style="max-width:450px;"><div class="mh"><h3>Settings</h3><button onclick="closeMo('setMo')">✕</button></div><div class="mbody"><b>Shop ID:</b> shop-001<br><b>Status:</b> Premium Active</div></div></div>
    <div id="ticketMo" class="mo hidden"><div class="mb" style="max-width:500px;"><div class="mh"><h3 id="tTitle">Raise Ticket</h3><button onclick="closeMo('ticketMo')">✕</button></div><div class="mbody"><input class="fi" id="tSubj" placeholder="Subject"><textarea class="fi" id="tDesc" rows="4" placeholder="Describe issue..."></textarea><button class="lpb" onclick="alert('Ticket successfully reported to management')">Submit Ticket</button></div></div></div>
    ...
    <script>
        ...
        async function handleLogin() {
            ...
            try {
                const r = await fetch(API + '/api/v1/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: e, password: p }) });
                const d = await r.json(); if (d.access_token) { localStorage.setItem('pl_s', d.access_token); window.location.reload(); }
                else lerr.textContent = "Invalid Credentials";
            } catch (e) { lerr.textContent = "Offline"; }
        }

        async function loadData() {
            try {
                const r = await fetch(API + '/api/v1/orders', { headers: { 'Authorization': 'Bearer ' + token } });
                allO = await r.json(); renderJobs(allO);
        ...
        function renderJobs(orders) {
            if (!orders.length) { jobsList.innerHTML = '<div style="padding:60px; text-align:center; color:#94a3b8; font-weight:800;">NO ACTIVE JOBS FOUND</div>'; return; }
            let h = '<table><tr><th>Order ID</th><th>Customer</th><th>Type</th><th>Document</th><th>Status</th></tr>';
            orders.slice(0, 8).forEach(o => { h += `<tr><td class="jid">#${o.order_number||o.id.slice(0,8)}</td><td><div style="font-weight:900;">${o.user_name||'Customer'}</div></td><td><span class="type_tag">${getT(o)}</span></td><td>${o.file_name||'Assignment_Final.pdf'}</td><td><span class="status_badge">${o.status}</span></td></tr>`; });
        ...
        function openPerf() {
            openMo('perfModal');
            pStats.innerHTML = `<div class="s_card"><span class="s_lbl">Total Orders</span><div class="s_val">${allO.length}</div></div><div class="s_card"><span class="s_lbl">Pending</span><div class="s_val" style="color:orange;">${allO.filter(o=>o.status=='pending').length}</div></div><div class="s_card"><span class="s_lbl">Processing</span><div class="s_val" style="color:blue;">2</div></div><div class="s_card"><span class="s_lbl">Delivered</span><div class="s_val" style="color:green;">1</div></div>`;
        ...
        function updateHistory(type, btn) {
            ...
            hList.innerHTML = dayData.reverse().map(item => `<div class="history_row"><div><div style="font-weight:900; font-size:1.1rem;">${item.label} <small style="color:#94a3b8;">${item.date}</small></div><div style="margin-top:8px; display:flex; gap:15px; flex-wrap:wrap;">${Object.keys(COLORS).map(c => `<span style="display:inline-flex; align-items:center; gap:5px; font-size:.75rem; color:#64748b; font-weight:800;"><span class="dot" style="background:${COLORS[c]};"></span> ${c} ₹${item.cats[c]}</span>`).join('')}</div></div><div style="font-weight:900; font-size:1.2rem;">₹${item.total.toLocaleString()}</div></div>`).join('');
        }

        async function openAgents() { openMo('agentMo'); agentList.innerHTML = "Fetching fleet..."; try { const r = await fetch(API + '/api/v1/delivery-persons', { headers: { 'Authorization': 'Bearer ' + token } }); const d = await r.json(); agentList.innerHTML = d.map(a => `<div style="padding:20px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center;"><div><b style="font-size:1.1rem;">${a.name}</b><br><small>${a.phone}</small></div><div style="background:#fde8e0; color:#e84c1e; padding:6px 14px; border-radius:12px; font-size:.85rem; font-weight:900;">${a.vehicle_number}</div></div>`).join(''); } catch(e) { agentList.innerHTML = "Offline."; } }
        function openTicketForm(t) { tTitle.innerText = t + ' Ticket Raise'; toggleLocalMenu('tdrop'); openMo('ticketMo'); }
