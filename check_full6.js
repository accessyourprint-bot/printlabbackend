...
function fmtDate(d){try{return new Date(d).toLocaleDateString('en-GB');}catch(e){return'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½';}}
function fmtTime(d){try{return new Date(d).toLocaleString('en-GB',{hour:'2-digit',minute:'2-digit',hour12:true});}catch(e){return'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½';}}
const PAGE_TITLES={
...
async function loadDelivery(){
  ...
  document.getElementById('dpList').innerHTML=agents.length?agents.map(a=>'<div class="dp-card"><div><div class="dp-name">'+a.name+'</div><div class="dp-meta">Age: '+(a.age||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+' &bull; +91 '+(a.phone||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</div><div class="dp-docs">Aadhar: '+(a.aadhar||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+' &bull; PAN: '+(a.pan||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</div><span class="badge b-blue" style="margin-top:6px;display:inline-block">'+(a.vehicle_number||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</span></div><button class="icon-del" onclick="delAgent(\''+a.id+'\')">&#128465;</button></div>').join(''):'<div class="empty">No delivery persons yet</div>';
}
...
function openResetPwModal(id){
  ...
  overlay.id='rpwOverlay';
  overlay.dataset.shopId=id;
  overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;z-index:9999';
  const _o = (typeof allOutlets!=='undefined' ? allOutlets.find(x=>x.id===id) : null);
  ...
  overlay.innerHTML=
    '<div style="background:#fff;border-radius:12px;padding:24px;width:340px;max-width:90vw">'+
      ...
      '<div class="fg"><label>New Password</label><div style="position:relative">'+
        '<input id="rpwNew" type="password" placeholder="Enter new password" style="width:100%;padding-right:44px">'+
        '<button type="button" onclick="togglePw(\'rpwNew\',\'rpwEye1\')" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;padding:4px">'+
          '<span id="rpwEye1">\ud83d\udc41</span></button></div></div>'+
      '<div class="fg" style="margin-top:12px"><label>Re-enter Password</label><div style="position:relative">'+
        '<input id="rpwConfirm" type="password" placeholder="Re-enter password" style="width:100%;padding-right:44px">'+
        '<button type="button" onclick="togglePw(\'rpwConfirm\',\'rpwEye2\')" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;padding:4px">'+
          '<span id="rpwEye2">\ud83d\udc41</span></button></div></div>'+
      '<p id="rpwErr" style="color:#c00;font-size:.8rem;margin:10px 0 0;display:none"></p>'+
      '<div style="display:flex;gap:8px;margin-top:18px">'+
        '<button onclick="document.getElementById(\'rpwOverlay\').remove()" style="flex:1;padding:10px;background:#eee;border:none;border-radius:8px;cursor:pointer">Cancel</button>'+
        '<button onclick="submitResetPw()" style="flex:1;padding:10px;background:#1a223e;color:#fff;border:none;border-radius:8px;cursor:pointer">Reset</button>'+
      '</div>'+
    '</div>';
...
function togglePw(inputId,eyeId){
  ...
  const s=document.getElementById(eyeId);
  if(p.type==='password'){p.type='text';s.textContent='\ud83d\ude48';}else{p.type='password';s.textContent='\ud83d\udc41';}
}
async function submitResetPw(){
  const overlay=document.getElementById('rpwOverlay');
  const id=overlay.dataset.shopId;
  const pw=document.getElementById('rpwNew').value;
  const pw2=document.getElementById('rpwConfirm').value;
  const err=document.getElementById('rpwErr');
  if(!pw||pw.length<6){err.textContent='Password must be at least 6 characters';err.style.display='block';return;}
  if(pw!==pw2){err.textContent='Passwords do not match';err.style.display='block';return;}
  try{
    const r=await api('/api/v1/shops/'+id+'/reset-password','PATCH',{new_password:pw});
    if(r && r.data && r.data.new_password){
      overlay.remove();
      alert('Password reset successfully.');
    }else{
      err.textContent='Password reset, but no confirmation was returned.';err.style.display='block';
    }
  }catch(e){err.textContent='Failed to reset password';err.style.display='block';}
}
function resetOutletPassword(id){ openResetPwModal(id); }
...
function filterOutlets(type,el){
  ...
  list.map(function(o){
    ...
    return '<div style="background:#fff;border:1.5px solid #ebebee;border-radius:12px;padding:16px">'+
      ...
      '<div><span style="color:#999;font-weight:600">Login ID:</span> '+(o.owner_email?o.owner_email.split('@')[0]:'Ã¯Â¿Â½')+'</div>'+
      '<div><span style="color:#999;font-weight:600">Owner:</span> '+(o.owner_name||'Ã¯Â¿Â½')+'</div>'+
      '<div><span style="color:#999;font-weight:600">Phone:</span> '+(o.owner_phone||'Ã¯Â¿Â½')+'</div>'+
      '<div><span style="color:#999;font-weight:600">Pincode:</span> '+(o.pincode||'Ã¯Â¿Â½')+'</div>'+
...
async function loadSubAdmins(){
  ...
  document.getElementById('saTable').innerHTML=allSubAdmins.length?
    ...
    allSubAdmins.map((u,i)=>'<tr><td class="uid">SA'+String(i+1).padStart(3,'0')+'</td><td>'+(u.full_name||u.name||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</td><td><span class="badge b-dark">'+(u.role||'sub-admin')+'</span></td><td>'+fmtDate(u.created_at)+'</td><td><button class="icon-del" onclick="delSubAdmin(\''+u.id+'\')">&#128465;</button></td></tr>').join('')+'</table>':
...
async function loadAccountMgmt(){
  ...
  document.getElementById('amTable').innerHTML=users.length?
    ...
    users.map((u,i)=>{const active=u.is_active!==false;return'<tr><td class="uid">CUS'+String(i+1).padStart(3,'0')+'</td><td>'+(u.full_name||u.name||'Customer')+'</td><td>+91 '+(u.phone||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</td><td><span class="badge '+(active?'b-green':'b-red')+'">'+(active?'active':'suspended')+'</span></td><td><button class="btn-sm '+(active?'btn-red':'btn-orange')+'" onclick="toggleUserStatus(\'+u.id+\','+(!active)+')">'+(active?'&#128683; Suspend':'&#128275; Activate')+'</button></td></tr>';}).join('')+'</table>':
...
async function loadStock(){
  ...
  document.getElementById('skTickets').innerHTML=tickets.length?tickets.map(t=>'<div style="border:1px solid #f0f0f3;border-radius:8px;padding:10px 12px;margin-bottom:8px"><div style="display:flex;justify-content:space-between"><span class="badge b-yellow">'+(t.status||'open')+'</span><span style="font-size:.76rem;color:#aaa">'+(t.category==='customer'?'Customer':'Outlet')+'</span></div><div style="font-weight:700;font-size:.88rem;margin-top:6px">'+(t.subject||'Issue')+'</div><div style="font-size:.78rem;color:#aaa">'+((t.raised_by||'').toString().slice(0,8)||'+Ã¯Â¿Â½+Ã¯Â¿Â½+Ã¯Â¿Â½')+'</div></div>').join(''):'<div class="empty">No open tickets</div>';
}
