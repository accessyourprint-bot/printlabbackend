...
async function loadPrintDone(){
  ...
  document.getElementById('pdTable').innerHTML=done.length?
    ...
    done.map((o,i)=>'<tr><td class="uid">'+(o.user_id?.toString().slice(0,8)||'USR')+'</td><td>'+(o.product_name||'Print Job')+'</td><td>'+(o.copies||1)+'</td><td><span class="badge b-green">done</span></td><td>'+fmtDate(o.completed_at||o.created_at)+', '+fmtTime(o.completed_at||o.created_at)+'</td></tr>').join('')+'</table>':
...
async function loadPaymentDay(){
  ...
  document.getElementById('pdayTable').innerHTML=todays.length?
    ...
    todays.map((o,i)=>'<tr><td class="uid">'+(o.user_id?.toString().slice(0,8)||'USR')+'</td><td>'+(o.user_name||'Customer')+'</td><td style="color:#1b7a3d;font-weight:700">&#8377;'+(parseFloat(o.grand_total)||0).toLocaleString('en-IN')+'</td><td><span class="badge b-dark">'+(o.payment_method||'upi')+'</span></td><td>'+fmtTime(o.created_at)+'</td></tr>').join('')+'</table>':
...
async function loadPaymentHist(){
  ...
  for(let w=0;w<byDay.length;w+=3){
    ...
    html+='<div class="panel" style="margin-bottom:14px"><div class="panel-hd">Week '+weekNum+'</div><table><tr><th>Day</th><th>User Name</th><th>User ID</th><th>Amount</th></tr>';
    chunk.forEach(d=>{d.orders.forEach(o=>{html+='<tr><td>'+d.date.getDate()+' '+['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][d.date.getMonth()]+'</td><td>'+(o.user_name||'Customer')+'</td><td class="uid">'+(o.user_id?.toString().slice(0,8)||'USR')+'</td><td style="color:#1b7a3d;font-weight:700">&#8377;'+(parseFloat(o.grand_total)||0).toLocaleString('en-IN')+'</td></tr>';});});
...
async function loadDelivery(){
  ...
  document.getElementById('dpList').innerHTML=agents.length?agents.map(a=>'<div class="dp-card"><div><div class="dp-name">'+a.name+'</div><div class="dp-meta">Age: '+(a.age||'—')+' &bull; +91 '+(a.phone||'—')+'</div><div class="dp-docs">Aadhar: '+(a.aadhar||'—')+' &bull; PAN: '+(a.pan||'—')+'</div><span class="badge b-blue" style="margin-top:6px;display:inline-block">'+(a.vehicle_number||'—')+'</span></div><button class="icon-del" onclick="delAgent(\''+a.id+'\')">&#128465;</button></div>').join(''):'<div class="empty">No delivery persons yet</div>';
}
...
async function loadCustomers(){
  ...
  document.getElementById('cuTable').innerHTML=allCustomers.length?
    ...
    allCustomers.map((u,i)=>'<tr><td class="uid">CUS'+String(i+1).padStart(3,'0')+'</td><td>'+(u.full_name||u.name||'Customer')+'</td><td>+91 '+(u.phone||'—')+'</td><td><span class="badge '+(u.is_active!==false?'b-green':'b-red')+'">'+(u.is_active!==false?'active':'suspended')+'</span></td><td><button class="icon-del" onclick="delCustomer(\''+u.id+'\')">&#128465;</button></td></tr>').join('')+'</table>':
...
function filterOutlets(type,el){
  ...
  document.getElementById('otTable').innerHTML=list.length?
    ...
    list.map((o,i)=>'<tr><td>'+(o.zone||o.address||'—')+'</td><td>'+(o.pincode||'—')+'</td><td>'+(o.owner_name||'—')+'</td><td class="uid">OUT'+String(i+1).padStart(3,'0')+'</td><td><span class="badge '+((o.type||'own')==='own'?'b-blue':'b-purple')+'">'+(o.type||'own')+'</span></td><td><button class="icon-del" onclick="delOutlet(\''+o.id+'\')">&#128465;</button></td></tr>').join('')+'</table>':
...
async function loadSubAdmins(){
  ...
  document.getElementById('saTable').innerHTML=allSubAdmins.length?
    ...
    allSubAdmins.map((u,i)=>'<tr><td class="uid">SA'+String(i+1).padStart(3,'0')+'</td><td>'+(u.full_name||u.name||'—')+'</td><td><span class="badge b-dark">'+(u.role||'sub-admin')+'</span></td><td>'+fmtDate(u.created_at)+'</td><td><button class="icon-del" onclick="delSubAdmin(\''+u.id+'\')">&#128465;</button></td></tr>').join('')+'</table>':
...
async function loadAccountMgmt(){
  ...
  document.getElementById('amTable').innerHTML=users.length?
    ...
    users.map((u,i)=>{const active=u.is_active!==false;return'<tr><td class="uid">CUS'+String(i+1).padStart(3,'0')+'</td><td>'+(u.full_name||u.name||'Customer')+'</td><td>+91 '+(u.phone||'—')+'</td><td><span class="badge '+(active?'b-green':'b-red')+'">'+(active?'active':'suspended')+'</span></td><td><button class="btn-sm '+(active?'btn-red':'btn-orange')+'" onclick="toggleUserStatus(\''+u.id+'\','+(!active)+')">'+(active?'&#128683; Suspend':'&#128275; Activate')+'</button></td></tr>';}).join('')+'</table>':
...
async function loadStock(){
  ...
  document.getElementById('skTickets').innerHTML=tickets.length?tickets.map(t=>'<div style="border:1px solid #f0f0f3;border-radius:8px;padding:10px 12px;margin-bottom:8px"><div style="display:flex;justify-content:space-between"><span class="badge b-yellow">'+(t.status||'open')+'</span><span style="font-size:.76rem;color:#aaa">'+(t.category==='customer'?'Customer':'Outlet')+'</span></div><div style="font-weight:700;font-size:.88rem;margin-top:6px">'+(t.subject||'Issue')+'</div><div style="font-size:.78rem;color:#aaa">'+((t.raised_by||'').toString().slice(0,8)||'—')+'</div></div>').join(''):'<div class="empty">No open tickets</div>';
}
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(HTML)
print('Done! Written', len(HTML), 'bytes to', path)
