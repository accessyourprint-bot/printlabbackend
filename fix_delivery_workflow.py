...
if start_idx == -1 or end_idx == -1:
  ...
  if(curStabFilter==='completed'){
    ...
    window.expandedRows = window.expandedRows || {};
    let rows=jobs.map(function(o){
      const isOpen = !!window.expandedRows[o.id];
      let mainRow = '<tr>'
...
function toggleDeliveryDetails(orderId){
  window.expandedRows = window.expandedRows || {};
  window.expandedRows[orderId] = !window.expandedRows[orderId];
  rJ(window.completedJobsCache || []);
}"""
    content = content[:start_idx] + new_fn + content[end_idx:]
    with io.open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("rJ function replaced with expand/reveal workflow")
