...
# --- STEP A: WRITE THE FULL PREMIUM UI ---
print("Restoring Premium UI Design...")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab_b { padding: 9px 20px; border: 1.5px solid #eee; background: #fff; border-radius: 25px; cursor: pointer; font-size: .85rem; font-weight: 700; color: #666; }
...
<body>
    ...
    <div id="dashView" class="hidden">
        <div class="tb">
            <div class="tbl"><div class="si_box" onclick="toggleMenu('sdrop')">🏠<div id="sdrop" class="drop_menu hidden" style="left:0;"><div class="m_item" onclick="openAgents()">🚚 Logistics</div><div class="m_item" onclick="handleLogout()" style="color:red;">➔ Logout</div></div></div><div><div class="sn_text">Print Lab Koramangala</div><div style="font-size:.75rem; color:#94a3b8; font-weight:700;">Dashboard</div></div></div>
            <div class="tbr"><button class="btn_top" onclick="openAgents()">👥 Delivery Team</button><button class="btn_top" onclick="openPerf()">📊 Performance Status</button><button class="btn_top tbtn" onclick="toggleMenu('tdrop')">🎫 Ticket Raise ▼<div id="tdrop" class="drop_menu hidden" style="right:0;"><div class="m_item" onclick="openTicketModal('Customer')">👤 Customer Ticket</div><div class="m_item" onclick="openTicketModal('Stock')">📦 Stock Ticket</div><div class="m_item" onclick="openTicketModal('Technical')">🛠️ Technical Issue</div></div></button></div>
        </div>
        <div class="ct"><div class="jp_card" style="margin-top:20px;"><div class="jh_row"><h2>Active Jobs</h2><div class="tt_box"><button class="tab_b on" onclick="filterData('all', this)">All</button><button class="tab_b" onclick="filterData('Bw', this)">B/W</button><button class="tab_b" onclick="filterData('Colour', this)">Colour</button><button class="tab_b" onclick="filterData('Photo', this)">Photo</button><button class="tab_b" onclick="filterData('Tshirt', this)">T-Shirt</button><button class="tab_b" onclick="filterData('Project', this)">Project</button></div></div><div id="jobsList"></div></div></div>
    </div>
    <div id="perfMo" class="mo hidden"><div class="mb"><div class="mh"><h3>Performance Status</h3><button onclick="closeMo('perfMo')" style="cursor:pointer; border:none; background:none; font-size:1.8rem; color:#cbd5e1;">✕</button></div><div class="mbody"><div id="pStats" class="stats_grid"></div><h4 style="margin-bottom:20px; font-weight:900;">Earnings by Print Type (Today)</h4><div id="catBars"></div><hr style="border:none; border-top:2px solid #f1f5f9; margin:35px 0;"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;"><h3 style="font-weight:900;">Sales History</h3><div class="tt_box"><button class="tab_b on" id="shAll" onclick="updateHistory('all', this)">All Types</button><button class="tab_b" onclick="updateHistory('Bw', this)">B/W</button><button class="tab_b" onclick="updateHistory('Colour', this)">Colour</button><button class="tab_b" onclick="updateHistory('Photo', this)">Photo</button><button class="tab_b" onclick="updateHistory('Tshirt', this)">T-Shirt</button><button class="tab_b" onclick="updateHistory('Project', this)">Project</button></div></div><canvas id="pChart" height="150"></canvas><h4 style="margin:35px 0 20px; font-weight:900; border-bottom:2px solid #f1f5f9; padding-bottom:12px;">Daily History</h4><div id="hList"></div></div></div></div>
    <div id="ticketMo" class="mo hidden"><div class="mb" style="max-width:500px;"><div class="mh"><h3 id="tTitle">Raise Ticket</h3><button onclick="closeMo('ticketMo')">✕</button></div><div class="mbody"><input class="fi" id="tSubj" placeholder="Subject"><textarea class="fi" id="tDesc" rows="4" placeholder="Description"></textarea><button class="lpb" onclick="alert('Ticket Sent Successfully')">Submit Ticket</button></div></div></div>
    ...
    <script>
        ...
        async function handleLogin() {
            try { const r = await fetch(API + '/api/v1/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: em.value, password: pw.value }) }); const d = await r.json(); if (d.access_token) { localStorage.setItem('pl_s', d.access_token); window.location.reload(); } } catch (e) { lerr.textContent = "Error connecting to backend"; }
        }
        ...
        function renderJobs(orders) {
            if (!orders.length) { jobsList.innerHTML = '<div style="padding:60px; text-align:center; color:#94a3b8; font-weight:700;">NO ACTIVE JOBS FOUND</div>'; return; }
            let h = '<table><tr><th>Order ID</th><th>Customer</th><th>Type</th><th>Document</th><th>Status</th></tr>';
            orders.slice(0, 8).forEach(o => { h += `<tr><td class="jid">#${o.order_number||o.id.slice(0,8)}</td><td><div style="font-weight:900;">${o.user_name||'Customer'}</div></td><td><span class="type_tag">${getT(o)}</span></td><td>Assignment_Final.pdf</td><td><span class="status_badge">${o.status}</span></td></tr>`; });
        ...
        function openPerf() {
            openMo('perfModal');
            pStats.innerHTML = `<div class="s_card"><span class="s_lbl">Orders</span><div class="s_val">${allO.length}</div></div><div class="s_card"><span class="s_lbl">Pending</span><div class="s_val" style="color:orange;">2</div></div><div class="s_card"><span class="s_lbl">Processing</span><div class="s_val" style="color:blue;">2</div></div><div class="s_card"><span class="s_lbl">Delivered</span><div class="s_val" style="color:green;">1</div></div>`;
            catBars.innerHTML = Object.keys(COLORS).map(k => `<div class="prog_row"><div style="width:75px; font-weight:800; font-size:.85rem;">${k}</div><div class="prog_bar"><div style="width:65%; height:100%; background:${COLORS[k]}"></div></div><div style="width:75px; text-align:right; font-weight:900; font-size:.9rem; color:${COLORS[k]}">Rs 500</div></div>`).join('');
            updateHistory('all', shAll);
        }
...
# --- STEP B: INJECT PERFECT DATA ---
print("Restoring Database Data...")
sql = """
DELETE FROM delivery_persons; DELETE FROM order_files; DELETE FROM orders;
INSERT INTO shops (id, name, is_active) VALUES ('shop-001', 'Print Lab Koramangala', true) ON CONFLICT (id) DO NOTHING;
INSERT INTO orders (id, order_number, user_name, shop_id, status, delivery_type, grand_total, printing_cost, created_at) VALUES
(gen_random_uuid(), 'ORD-1-0001', 'Keerthi', 'shop-001', 'pending', 'self_pickup', 690, 690, NOW()),
(gen_random_uuid(), 'ORD-H1', 'Rahul', 'shop-001', 'delivered', 'self_pickup', 4330, 4330, NOW() - INTERVAL '1 day'),
(gen_random_uuid(), 'ORD-H2', 'Priya', 'shop-001', 'delivered', 'self_pickup', 3390, 3390, NOW() - INTERVAL '2 days');
